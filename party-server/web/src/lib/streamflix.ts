...
const isPartyStreamUri = (uri: string): boolean =>
  /^https?:\/\//i.test(uri) && STREAM_URI_RE.test(uri);

const partySourceKind = (uri: string): 'hls' | 'file' =>
...
const extractFetch = async (
  ...
  try {
    return await fetch(`/extract?u=${encodeURIComponent(url)}`, {
      ...init,
      headers: {
...
const pickAtlasMp4 = async (listUrl: string): Promise<string | null> => {
  try {
    ...
    const res = await extractFetch(listUrl);
    if (!res.ok) {
...
const extractVideasy = async (
  ...
  const encRes = await extractFetch(source.extractUrl);
  if (!encRes.ok) {
  ...
  const decRes = await extractFetch(VIDEASY_DEC, {
    method: 'POST',
...
const getVidzeeMasterKey = async (): Promise<string | null> => {
  ...
  try {
    const res = await extractFetch(`${VIDZEE_CORE}/api-key`);
...
const extractVidzee = async (
  ...
  const res = await extractFetch(source.extractUrl);
  if (!res.ok) {
...
export const toWebSource = (source: StreamflixSource) => ({
  id: source.id,
  name: source.name,
...
export const registerSubtitleUrl = async (session: string, url: string): Promise<void> => {
